...
import os
from tensorflow.keras import backend, layers, models, utils as keras_utils
...
def block1(x, filters, kernel_size=3, stride=1,
           conv_shortcut=True, name=None, actfun='relu'):
    """A residual block.
    ...
    if conv_shortcut is True:
        shortcut = layers.Conv2D(4 * filters, 1, strides=stride,
                                 name=name + '_0_conv')(x)
        shortcut = batch_norm_agg(axis=bn_axis, epsilon=1.001e-5,
    ...
    x = batch_norm_agg(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_1_bn')(x)
    x = layers.Activation(actfun, name=name + '_1_' + actfun)(x)

    x = layers.Conv2D(filters, kernel_size, padding='SAME',
                      name=name + '_2_conv')(x)
    x = batch_norm_agg(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_2_bn')(x)
    x = layers.Activation(actfun, name=name + '_2_' + actfun)(x)
    ...
    x = layers.Activation(actfun, name=name + '_out')(x)
    return x
...
def block2(x, filters, kernel_size=3, stride=1,
           conv_shortcut=False, name=None, actfun = 'relu'):
    """A residual block.
    ...
    preact = batch_norm_agg(axis=bn_axis, epsilon=1.001e-5,
                                       name=name + '_preact_bn')(x)
    preact = layers.Activation(actfun, name=name + '_preact_' + actfun)(preact)
...
def stack2(x, filters, blocks, stride1=2, name=None, actfun='relu'):
    """A set of stacked residual blocks.
    ...
    # Returns
        Output tensor for the stacked blocks.
    """
    x = block2(x, filters, conv_shortcut=True, name=name + '_block1', actfun=actfun)
    # for i in range(2, blocks):
    #     x = block2(x, filters, name=name + '_block' + str(i), actfun=actfun)
    x = block2(x, filters, stride=stride1, name=name + '_block' + str(blocks), actfun=actfun)
    return x
...
def ResNet152V2(include_top=True,
    ...
    def stack_fn(x):
        x = stack2(x, 64, 3, name='conv2', actfun=actfun)
        x = stack2(x, 128, 8, name='conv3', actfun=actfun)
        x = stack2(x, 256, 36, name='conv4', actfun=actfun)
        x = stack2(x, 512, 3, stride1=1, name='conv5', actfun=actfun)
        return x
